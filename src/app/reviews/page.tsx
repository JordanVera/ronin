    ...
    <>
      <section className="relative h-64 sm:h-80 overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-6 pt-20">
          <p className="text-[#f45235] text-[10px] tracking-[0.4em] uppercase mb-4">Guest Love</p>
      ...
      <section className="py-20 px-6 lg:px-8 max-w-7xl mx-auto">
            ...
            <blockquote key={review.id} className="bg-card border border-border p-8">
              <div className="flex gap-1 mb-4">
                {Array.from({ length: review.rating }).map((_, n) => (
                  <Star key={n} size={14} className="fill-[#f45235] text-[#f45235]" />
                ))}
              </div>
        ...
        <div className="mt-16 text-center">
          <a href={COMPANY.googleReviewsUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-3.5 bg-[#f45235] text-white text-xs tracking-[0.2em] uppercase font-medium hover:opacity-90 transition-opacity">
